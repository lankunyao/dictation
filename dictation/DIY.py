...
allDefaultVal = [QColor(0, 240, 0), QColor(0, 160, 0), QColor(0, 68, 0), QColor(0, 28, 0), QColor(140, 140, 140),
                 QColor(100, 100, 100), 500, 450, 400]
allLabelNames = [u'灯亮圆心颜色：', u'灯亮边缘颜色：', u'灯灭圆心颜色：', u'灯灭边缘颜色：', u'边框内测颜色：', u'边框外侧颜色：', u'边框外侧半径：', u'边框内侧半径：',
                 u'中间圆灯半径：']


class QPushButton_circle(QAbstractButton):
    def __init__(self):
        super().__init__()
    ...
    def initUI(self):
        ...
        self.setButtonDefaultOption()

    def setButtonDefaultOption(self):
        for attr, val in zip(allAttributes, allDefaultVal):
            setattr(self, attr, val)
        self.update()

    def setButtonOption(self, opt='colorOnBegin', val=QColor(0, 240, 0)):
        if hasattr(self, opt):
